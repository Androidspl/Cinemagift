...
public class InfoViewPagerAdapter extends PagerAdapter {
	...
	public InfoViewPagerAdapter(Context context, List<Banner> vList) {
	...
	public Object instantiateItem(View view, final int position) {
		try {
			...
			if (v == null) {
				...
				banner_text.setText(datas.get(position).title);
				v.setTag(imageview);
				mList[position] = v;
			} else {
				imageview = (ImageView) v.getTag();
			}
			loader.displayImage(Const.BASE_URL + "/" + datas.get(position).img_url, imageview, options);
			v.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					Intent intent = new Intent(mContext, WebViewActivity.class);
					intent.putExtra("url", datas.get(position).link_url);
